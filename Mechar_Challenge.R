> ### Deliverable 1 ###
> # Import the data
> mecha_car_mpg <-read.csv(file='MechaCar_mpg.csv', check.names =F, stringAsFactors = F)
...
> ### Deliverable 2 ###
> # Import the data
> susp_coil <- read.csv(file='Suspension_Coil.csv', check.names = F, stringAsFactors =F)
Error in read.table(file = file, header = header, sep = sep, quote = quote,  : 
  unused argument (stringAsFactors = FALSE)
...
Error in summarize(., Mean = mean(PSI), Median = median(PSI), Variance = var(PSI),  : 
  object 'sus_coil' not found
